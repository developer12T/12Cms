<template>
    <LayoutSub>
        <template v-slot>
            <div class="flex flex-col md:h-full space-y-4 p-4">
                <Alert v-if="showAlertCancel" :title="'ยกเลิกรายการขาย'" :content="'ยืนยันการยกเลิกรายการขาย'"
                    @confirm="handleCancel" @dismiss="showAlertCancel = false"
                ...
                <div class="flex flex-col items-center">
                    <div class="bg-white px-4 py-2 shadow-md rounded-lg overflow-auto h-[230px] w-full max-w-2xl">
                        <div class="flex flex-col space-y-2">
                            <div v-for="list in detailList" :key="list.index" class="border-b pb-2">
    ...
    <AlertBluetooth v-if="showAlertPrint" :title="'เชื่อมต่อบลูทูธ'" @print="handlePrint"
        @dismiss="showAlertPrint = false" :color="'text-gray-600 border border-green-300 bg-green-100'" />
...
const showAlertPrint = ref(false)
...
const handleCancel = async (orderNo) => {
    ...
    try {
        await orderStore.updateOrder({
            order: detail.value.orderNo,
            status: '90',
        })
        showAlertCancel.value = false;
