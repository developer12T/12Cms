...
const detail = computed(() => order.returnDetail)
const saleitem = computed(() => order.saleItem)
const returnitem = computed(() => order.returnItem)
const customer = computed(() => order.returnCustomer)
...
onMounted(async () => {
    const CUOR = localStorage.getItem('CUOR')
    await order.getReturnDetail(CUOR)
    loading.value = false
});
</script>
